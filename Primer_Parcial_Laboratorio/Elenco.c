    ...
    if(ingresoSecuencialValido==1)
    {
        ...
        if(ingresoSecuencialValido==1)
        {
            ...
            if(getStrSoloNumeroSinRango("\nIngrese el codigo de un actor/a: ",auxCodigoActorStr,"\nSolo se permite el numero de un actor/a\n",3))
            {
                ...
                indiceActor=buscarActorEnElElenco(listaElencos,tamElenco,auxDatos.codigoActor);
...
{
    printf("\nActor: %s",listaActores[indiceActor].nombre);
...
int buscarActorEnElElenco(eElenco listaElencos[],int tamElenco,int codigoActorEnElenco)
{
    ...
    for(i=0;i<tamElenco;i++)
    {
        if((listaElencos[i].estado==OCUPADO) && (listaElencos[i].codigoActor==codigoActorEnElenco))
        {
            retorno=i;
            break;
        }
