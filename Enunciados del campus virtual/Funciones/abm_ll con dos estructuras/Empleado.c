...
int empleado_setIdLocalidad(eEmpleado* empleado,int idLocalidad)
{
    int retorno=0;
    if((empleado!=NULL) && (idLocalidad > 0))
    {
        empleado->id_localidad=idLocalidad;
        retorno=1;
    }
    return retorno;
}
...
int empleado_getIdLocalidad(eEmpleado* empleado,int* idLocalidad)
{
    int retorno=0;
    if(empleado!=NULL && idLocalidad!=NULL)
    {
        *idLocalidad=empleado->id_localidad;
        retorno=1;
    }
    return retorno;
}
eEmpleado* empleado_nuevoEmpleadoParametros(char* id,char* nombre,char* apellido,char* edad,char* sueldo,char* idLocalidad)
{
    eEmpleado* unEmpleado=NULL;
    if((id!=NULL) && (nombre!=NULL) && (apellido!=NULL) && (edad!=NULL) && (sueldo!=NULL) && (idLocalidad!=NULL))
    {
        unEmpleado=empleado_nuevoEmpleado();
        if(unEmpleado!=NULL)
        {
            if(!empleado_setId(unEmpleado,atoi(id)) || !empleado_setNombre(unEmpleado,nombre) || !empleado_setApellido(unEmpleado,apellido) || !empleado_setEdad(unEmpleado,atoi(edad)) || !empleado_setSueldo(unEmpleado,atof(sueldo)) || !empleado_setIdLocalidad(unEmpleado,atoi(idLocalidad)))
            {
...
int empleado_mostrarUnEmpleado(eEmpleado* unEmpleado,eLocalidad* unaLocalidad)
{
    int seMostro=0;
    if(unEmpleado!=NULL && unaLocalidad!=NULL)
    {
        printf("\n%d",unEmpleado->id);
        printf("%14s",unEmpleado->nombre);
        printf("%14s",unEmpleado->apellido);
        printf("%7d",unEmpleado->edad);
        printf("%12.2f",unEmpleado->sueldo);
        printf("%12s\n",unaLocalidad->descripcion);
        seMostro=1;
    }
    return seMostro;
}
void empleado_mostrarListaEmpleados(LinkedList* listaEmpleados,LinkedList* listaLocalidad)
{
    int indiceBusqueda;
    ...
    if(listaEmpleados!=NULL && listaLocalidad!=NULL)
    {
        ...
        {
            printf("\nID       NOMBRE      APELLIDO     EDAD   SUELDO   LOCALIDAD\n");
            for(i=0;i<len;i++)
            {
                auxEmpleado=(eEmpleado*)ll_get(listaEmpleados,i);
                indiceBusqueda=localidad_buscarPorId(listaLocalidad,auxEmpleado->id_localidad);
                if(indiceBusqueda!=-1)
                {
                    auxLocalidad=(eLocalidad*)ll_get(listaLocalidad,indiceBusqueda);
                    if(auxEmpleado->id_localidad==auxLocalidad->id)
                    {
                        empleado_mostrarUnEmpleado(auxEmpleado,auxLocalidad);
                    }
                }
            }
        }
...
int empleado_darDeAlta(LinkedList* listaEmpleados,LinkedList* listaLocalidad,int* contAltas)
{
    int sePudo=-1;
    int indiceBusqueda;
    int confirmacion;
    ...
    if(listaEmpleados!=NULL)
    {
        ...
        localidad_mostrarListaLocalidades(listaLocalidad);
        ...
        {
            indiceBusqueda=localidad_buscarPorId(listaLocalidad,atoi(auxIdLocalidadStr));
            if(indiceBusqueda==-1)
            {
        ...
        if(ingresoSecuencialValido==1)
        {
            ...
            if(confirmacion==1)
            {
                ...
                *contAltas=auxIdInt+1;
                auxDatos=empleado_nuevoEmpleadoParametros(auxIdStr,auxNombre,auxApellido,auxEdadStr,auxSueldoStr,auxIdLocalidadStr);
...
int empleado_darDeBaja(LinkedList* listaEmpleados,LinkedList* listaLocalidad)
{
    int sePudo=-1;
    int confirmacion;
    ...
    {
        empleado_mostrarListaEmpleados(listaEmpleados,listaLocalidad);
...
int empleado_modificarDatos(LinkedList* listaEmpleados,LinkedList* listaLocalidad)
{
    int sePudo=-1;
    int indiceResultadoBusqueda;
    char auxIdStr[256];
    ...
    else
    {
        ...
        if(getStrNumerosSinRango("\nIngrese el id a modificar: ",auxIdStr,"\nSolo se permiten numeros\n",3))
        {
            ...
            {
                empleado_pedirDatosAModificar(listaEmpleados,listaLocalidad,indiceResultadoBusqueda);
                sePudo=1;
            }
        }
        if(sePudo==-1)
...
void empleado_pedirDatosAModificar(LinkedList* listaEmpleados,LinkedList* listaLocalidad,int indiceEncontrado)
{
    int confirmacion;
    int indiceBusquedaLocalidad;
    eEmpleado* datosActuales=NULL;
    eEmpleado auxDatos;
    eEmpleado* proximosDatos=NULL;
    eLocalidad* localidadActual=NULL;
    eLocalidad* localidadAux=NULL;
    ...
    auxDatos=*datosActuales;

    int opcionMenu;
    char continuarMenu='s';
    do
    {
        indiceBusquedaLocalidad=localidad_buscarPorId(listaLocalidad,datosActuales->id_localidad);
        localidadActual=(eLocalidad*)ll_get(listaLocalidad,indiceBusquedaLocalidad);

        indiceBusquedaLocalidad=localidad_buscarPorId(listaLocalidad,auxDatos.id_localidad);
        localidadAux=(eLocalidad*)ll_get(listaLocalidad,indiceBusquedaLocalidad);

        system("cls");
        printf("\nID       NOMBRE      APELLIDO     EDAD   SUELDO   LOCALIDAD\n");
        printf("\nDatos actuales seleccionados:\n");
        empleado_mostrarUnEmpleado(datosActuales,localidadActual);
        printf("\nDatos a modificar:\n");
        empleado_mostrarUnEmpleado(&auxDatos,localidadAux);
        ...
        switch(opcionMenu)
        {
            ...
            case 5:
                ...
                if(getStrNumerosSinRango("\nIngrese el id de una localidad: ",auxIdLocalidadStr,"\nSolo se permiten numeros\n",3))
                {
                    indiceBusquedaLocalidad=localidad_buscarPorId(listaLocalidad,atoi(auxIdLocalidadStr));
                    if(indiceBusquedaLocalidad!=-1)
                    {
                        auxDatos.id_localidad=atoi(auxIdLocalidadStr);
                        printf("\nSe ha ingresado el id\n");
                        flagDatoIngresado=1;
                    }
                    else
                    {
...
int empleado_guardarDatosModoTexto(FILE* archivo,LinkedList* listaEmpleados,int* contAltas)
{
    ...
    else
    {
        ...
        if(archivo!=NULL)
        {
            ...
            fprintf(archivo,"%s\n",auxAltasStr);
            fprintf(archivo,"ID;NOMBRE;APELLIDO;EDAD;SUELDO;ID_LOCALIDAD\n");
            for(i=0;i<len;i++)
            {
                ...
                itoa(auxDatos->id_localidad,auxIdLocalidadStr,10);
                fprintf(archivo,"%s;%s;%s;%s;%s;%s\n",auxIdStr,auxNombreStr,auxApellidoStr,auxEdadStr,auxSueldoStr,auxIdLocalidadStr);
            }
            fclose(archivo);
...
int empleado_cargarDatosModoTexto(FILE* archivo,LinkedList* listaEmpleados,int* contAltas)
{
    ...
    if(archivo!=NULL)
    {
        ...
        *contAltas=atoi(auxAltasStr);
        ...
        {
            fscanf(archivo,"%[^;];%[^;];%[^;];%[^;];%[^;];%[^\n]\n",auxIdStr,auxNombreStr,auxApellidoStr,auxEdadStr,auxSueldoStr,auxIdLocalidadStr);
            auxDatos=empleado_nuevoEmpleadoParametros(auxIdStr,auxNombreStr,auxApellidoStr,auxEdadStr,auxSueldoStr,auxIdLocalidadStr);
            ll_add(listaEmpleados,auxDatos);
            if(feof(archivo))
            {
                break;
