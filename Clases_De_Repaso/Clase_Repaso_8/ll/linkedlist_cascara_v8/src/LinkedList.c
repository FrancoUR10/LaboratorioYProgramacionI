 ...
 * \return int Retorna  (-1) Error: si alguno de los punteros a las listas son NULL
                        ( 1) Si los elementos de (lista2) estan contenidos en la lista (this)
                        ( 0) si los elementos de (lista2) NO estan contenidos en la lista (this)
*/
	...
	if(lista!=NULL && pFunc != NULL && (order==1 || order==0))
    {
        ...
        for(i=0;i<len-1;i++)
        {
            for(j=i+1;j<len;j++)
            {
                ...
                if(pFunc(ll_get(lista,i), ll_get(lista,j))==criterio)
                {
